package com.aicreator.offline.ui.screens.face
...
@Composable
fun FaceModeScreen(container: AppContainer, onDone: () -> Unit) {
    val selection by container.characterSelectionHolder.selection.collectAsStateWithLifecycle()

    Column(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
        Text("Ritratto e volto", style = MaterialTheme.typography.titleLarge)
        Text(container.faceConditioning.techniqueDescription, style = MaterialTheme.typography.bodyMedium, modifier = Modifier.padding(top = 8.dp))

        val current = selection
        if (current == null || current.imagePath == null) {
            Text(
                "Nessuna foto di riferimento selezionata. Vai su \"Personaggio\" e scegli \"Usa: Volto\" su un personaggio salvato.",
