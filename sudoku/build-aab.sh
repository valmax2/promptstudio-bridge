...
# cioè bash setup-android.sh).
#
# Uso normale:
#   cd sudoku
#   bash build-aab.sh
#
# Legge la firma da sudoku/keystore.properties (NON versionato):
...
if [ ! -d android ]; then
  echo "✗ Cartella 'android/' assente. Esegui prima: bash setup-android.sh"
  exit 1
fi
...
mkdir -p www
cp index.html privacy.html styles.css sudoku-engine.js ads.js billing.js app.js \
